...
float ring(vec2 p,float ri,float cut,float t0,float px){
  float t=mod(uTime+t0,CYCLE);float r=ri+t/CYCLE*uScaleRate;float d=abs(length(p)-r);
  float a=atan(abs(p.y),abs(p.x))/HP;float th=max(1.0-a,0.5)*px*uLineThickness;
  float h=(1.0-smoothstep(th,th*1.5,d))+1.0;d+=pow(cut*a,3.0)*r;
  return h*exp(-uAttenuation*d)*fade(t);
}
void main(){
  float px=1.0/min(uResolution.x,uResolution.y);
  vec2 p=(gl_FragCoord.xy-0.5*uResolution.xy)*px;
  float cr=cos(uRotation),sr=sin(uRotation);p=mat2(cr,-sr,sr,cr)*p;
  p-=uMouse*uMouseInfluence;
  float sc=mix(1.0,uHoverScale,uHoverAmount)+uBurst*0.3;p/=sc;
  vec3 c=vec3(0.0);float rcf=max(float(uRingCount)-1.0,1.0);
  for(int i=0;i<10;i++){
    if(i>=uRingCount)break;float fi=float(i);
...
export default function MagicRings({
  color = "#8b5cf6", colorTwo = "#22d3ee", speed = 1, ringCount = 6,
  ...
  useEffect(() => {
    ...
    const onMove = (e: MouseEvent) => { const r = mount.getBoundingClientRect(); mouseRef.current = [(e.clientX - r.left) / r.width - 0.5, -((e.clientY - r.top) / r.height - 0.5)]; };
    ...
    const animate = (t: number) => {
      ...
      (uniforms.uColor.value as THREE.Color).set(p.color || "#8b5cf6");
      (uniforms.uColorTwo.value as THREE.Color).set(p.colorTwo || "#22d3ee");
    ...
    return () => {
      ...
      ro.disconnect(); mount.removeEventListener("mousemove", onMove);
      mount.removeEventListener("mouseenter", onEnter); mount.removeEventListener("mouseleave", onLeave);
      mount.removeEventListener("click", onClick); mount.removeChild(renderer.domElement);
