...
import React, {
  Children, cloneElement, forwardRef, isValidElement, ReactElement, ReactNode, RefObject, useEffect, useMemo, useRef
} from "react";
import gsap from "gsap";

export interface CardSwapProps {
  width?: number | string; height?: number | string; cardDistance?: number; verticalDistance?: number;
  delay?: number; pauseOnHover?: boolean; onCardClick?: (idx: number) => void; skewAmount?: number;
...
const CardSwap: React.FC<CardSwapProps> = ({
  width = 500, height = 400, cardDistance = 60, verticalDistance = 70, delay = 5000,
  pauseOnHover = false, onCardClick, skewAmount = 6, easing = "elastic", children
}) => {
  ...
  const rendered = childArr.map((child, i) =>
    isValidElement<CardProps>(child) ? cloneElement(child, {
  ...
  return (
    <div ref={container}
      className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 origin-center overflow-visible max-md:scale-75 max-sm:scale-[0.55]"
