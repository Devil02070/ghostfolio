...
float vWisps(vec2 uv,float topF){
  ...
  for(int s=0;s<2;++s){float sgn=s==0?-1.0:1.0;
    for(int i=0;i<W_LANES;++i){if(i>=lanes)break;
...
void mainImage(out vec4 fc,in vec2 frag){
  ...
  for(int k=-TAP_RADIUS;k<=TAP_RADIUS;++k){float tu=tH+float(k)*DT_LOCAL,wt=tauWf(tu,tauMin,tauMax);if(wt<=0.0)continue;
    float spd=max(abs(sin(tu)),0.02),u=clamp((basePhase-tu)/max(uDecay,EPS),0.0,1.0),env=pow(1.0-abs(u*2.0-1.0),0.8);
    vec2 p=vec2((R_H*uHLenFactor)*cos(tu),0.0);a+=wt*bs(uvc,p,env*spd);}
  float yPix=uvc.y,cy=clamp(-yPix/(R_V*uVLenFactor),-1.0,1.0),tV=clamp(TWO_PI-acos(cy),tauMin,tauMax);
  for(int k=-TAP_RADIUS;k<=TAP_RADIUS;++k){float tu=tV+float(k)*DT_LOCAL,wt=tauWf(tu,tauMin,tauMax);if(wt<=0.0)continue;
...
#if FOG_ON
  vec2 fuv=uvc*uFogScale;float mAct=step(1.0,length(iMouse.xy)),nx=((iMouse.x-C.x)*invW)*mAct;
  float ax=abs(nx);float stMag=mix(ax,pow(ax,FOG_TILT_SHAPE),0.35);float st=sign(nx)*stMag*uTiltScale;
  st=clamp(st,-FOG_TILT_MAX_X,FOG_TILT_MAX_X);vec2 dir=normalize(vec2(st,1.0));
  fuv+=uFogTime*uFogFallSpeed*dir;vec2 prp=vec2(-dir.y,dir.x);fuv+=prp*(0.08*sin(dot(uvc,prp)*0.08+uFogTime*0.9));
  float n=fbm2(fuv+vec2(fbm2(fuv+vec2(7.3,2.1)),fbm2(fuv+vec2(-3.7,5.9)))*0.6);n=pow(clamp(n,0.0,1.0),FOG_CONTRAST);
  ...
  float radialFade=1.0-smoothstep(0.0,0.7,length(uvc)/120.0);fog=n*browserFogIntensity*bBias*bm*hW*radialFade;
#endif
  float LF=L+fog;float dith=(h21(frag)-0.5)*(DITHER_STRENGTH/255.0);float tone=g(LF+w);
  vec3 col=tone*uColor+dith;float alpha=clamp(g(L+w*0.6)+dith*0.6,0.0,1.0);
  float nxE=abs((frag.x-C.x)*invW),xF=pow(clamp(1.0-smoothstep(EDGE_X0,EDGE_X1,nxE),0.0,1.0),EDGE_X_GAMMA);
  float scene=LF+max(0.0,w)*0.5,hi=smoothstep(EDGE_LUMA_T0,EDGE_LUMA_T1,scene);float eM=mix(xF,1.0,hi);
  col*=eM;alpha*=eM;col*=uFade;alpha*=uFade;fc=vec4(col,alpha);
}
...
const LaserFlow: React.FC<Props> = ({
  ...
  horizontalSizing = 0.5, fogIntensity = 0.45, fogScale = 0.3, wispSpeed = 15.0,
  wispIntensity = 5.0, flowStrength = 0.25, decay = 1.1, falloffStart = 1.2,
  fogFallSpeed = 0.6, color = '#3690d2'
}) => {
  ...
  useEffect(() => {
    ...
    const renderer = new THREE.WebGLRenderer({ antialias: false, alpha: true, depth: false, stencil: false, powerPreference: 'high-performance', premultipliedAlpha: false });
    ...
    const material = new THREE.RawShaderMaterial({ vertexShader:VERT, fragmentShader:FRAG, uniforms, transparent:true, depthTest:false, depthWrite:false });
    ...
    const io=new IntersectionObserver(e=>{inViewRef.current=e[0]?.isIntersecting??true;},{root:null,threshold:0}); io.observe(mount);
    const onVis=()=>{pausedRef.current=document.hidden;};
    document.addEventListener('visibilitychange',onVis,{passive:true});

    const updateMouse=(cx:number,cy:number)=>{const rect=rectRef.current;if(!rect)return;const x=cx-rect.left,y=cy-rect.top,r=currentDprRef.current;mouseTarget.set(x*r,(rect.height*r)-(y*r));};
    ...
    const animate=()=>{
      ...
      (uniforms.uFlowTime.value as number)+=cdt; (uniforms.uFogTime.value as number)+=cdt;
      ...
      mouseSmooth.lerp(mouseTarget,alpha); (uniforms.iMouse.value as THREE.Vector4).set(mouseSmooth.x,mouseSmooth.y,0,0);
      renderer.render(scene,camera);

      // adaptive dpr
      const now=performance.now();if(now-lastFpsCheckRef.current>750){
        const s=fpsSamplesRef.current;if(s.length>0){const avg=s.reduce((a,b)=>a+b,0)/s.length;
        let next=currentDprRef.current;if(avg<50)next=Math.max(0.6,next*0.85);else if(avg>58&&next<baseDprRef.current)next=Math.min(baseDprRef.current,next*1.1);
  ...
  useEffect(() => {
    ...
    const{r,g,b}=hexToRGB(color||'#FFFFFF');(u.uColor.value as THREE.Vector3).set(r,g,b);
